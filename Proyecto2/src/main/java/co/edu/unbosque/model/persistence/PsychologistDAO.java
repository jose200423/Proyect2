...
public class PsychologistDAO implements CRUDoperation {
	...
	public int updateByCcAttrs(long cc, String name, long cedula, Date cumple, String city, Date graduation, int days,
			int sessions, int salary) {
		LocalDate graduationDate = cumple.toLocalDate();
		LocalDate currentDate = LocalDate.now();
		long diasDesdeGraduacion = ChronoUnit.DAYS.between(graduationDate, currentDate);
		try {
			dbcon.initConnection();
			...
			dbcon.getPreparedstatement().setDate(3, cumple);
			dbcon.getPreparedstatement().setString(4, city);
			dbcon.getPreparedstatement().setDate(5, graduation);

			dbcon.getPreparedstatement().setInt(6, days);
			dbcon.getPreparedstatement().setInt(7, sessions);
			dbcon.getPreparedstatement().setInt(8, salary);
		...
		for (int i = 0; i < listpys.size(); i++) {
			if (listpys.get(i).getIdentificationNumber() == cc) {
				listpys.get(i).setName(name);
				listpys.get(i).setIdentificationNumber(cc);
				listpys.get(i).setBirthday(cumple);
				listpys.get(i).setCityOfBorn(city);
				listpys.get(i).setGraduationYear(graduation);
				listpys.get(i).setDaysSinceGraduation((int) diasDesdeGraduacion);
				listpys.get(i).setSalary(sessions);
				listpys.get(i).setSupportedSessions(salary);
